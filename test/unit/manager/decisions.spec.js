...
describe('DecisionsManager for stored decisions', () => {
  ...
  it('should return an empty string is nothing is in the cookie jar', function () {
    const resolutionResult = decisions.resolve({}, storage, emitter)
...
describe('DecisionsManager for new decisions', () => {
  ...
  it('should return the new decision id, and store it', function () {
    const decisionId = uuid()
    const resolutionResult = decisions.resolve({ pageUrl: `http://subdomain.tests.example.com/cake?li_did=${decisionId}` }, storage, emitter)
    expect(resolutionResult.decisionIds).to.eql([decisionId])
  ...
  it('should combine the new cookie and the stored one', function () {
    ...
    const resolutionResult = decisions.resolve({ pageUrl: `http://subdomain.tests.example.com/cake?li_did=    ${decisionIdTwo}` }, storage, emitter)
  ...
  it('should combine the new cookies and the stored one', function () {
    ...
    storage.setCookie('lidids.123', decisionIdOne)
    const resolutionResult = decisions.resolve({ pageUrl: `http://subdomain.tests.example.com/cake?li_did=${decisionIdTwo}&li_did=${decisionIdThree}` }, storage, emitter)
